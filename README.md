...
* **Приватные туннели** (`stcp`) — SSH, RDP, VNC. На сервере не открывается ни
  ...
  без секрета нельзя, найти сканированием — тоже. Взамен на машине, с которой
  подключаетесь, должна работать клиентская часть.
* **Публичный SSH** (`--public-ssh`) — то же, что `ngrok tcp 22`: сервер
  открывает порт в интернет и выдаёт адрес вида `ваш-сервер:6012`. Подключаться
  можно откуда угодно обычным `ssh -p 6012 пользователь@ваш-сервер`, ничего
  ставить на своей стороне не нужно. Порт выбирает сервер; чтобы он не менялся
  при переподключении, задайте его явно: `--public-ssh 6002`.
...
Публичный SSH удобен, но у него есть цена: ваш sshd становится виден из
интернета и получит свою долю переборщиков паролей. На такой машине оставьте
вход только по ключам (`PasswordAuthentication no` в `sshd_config`).
